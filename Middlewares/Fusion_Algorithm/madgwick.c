...
void MadgwickAHRSUpdate(MadgwickFilter_t *filter, float *gxyz_radps, float *axyz, float *mxyz, float dt){
	...
	float gx = gxyz_radps[0], gy = gxyz_radps[1], gz = gxyz_radps[2];
	float ax = axyz[0], ay = axyz[1], az = axyz[2];
	float mx = mxyz[0], my = mxyz[1], mz = mxyz[2];
    ...
    if (!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
        ...
        {
            float sumSq = s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3;
            if (sumSq > 0.0f) {
                recipNorm = invSqrt(sumSq);
                s0 *= recipNorm;
                s1 *= recipNorm;
                s2 *= recipNorm;
                s3 *= recipNorm;
                             ...
                             float *rollDeg, float *pitchDeg, float *yawDeg)
{
    ...
    *pitchDeg = asinf(sinp) * RAD_TO_DEG;
    ...
    float yawdeg1 = atan2f(2.0f * (q0 * q3 + q1 * q2), 1.0f - 2.0f * (q2 * q2 + q3 * q3)) * RAD_TO_DEG;
    if (yawdeg1 < 0) {
    	yawdeg1 += 360.0f;
    }
    *yawDeg   = yawdeg1;
